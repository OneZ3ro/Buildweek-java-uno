...
//        creazioneDocumentoVendita(dd, ud, mzd, rndm); //nella parte di creazione della tessera che cos'è dataDiEmissione e cosa cambia con dataDiRilascio
//        creazioneManutenzione(mtd, mzd, rndm);
    ...
    public static void creazioneUtente(UtenteDAO ud, Faker faker, Random rndm){
        for (int i = 0; i < rndm.nextInt(50, 101); i++) {
            Utente utente = new Utente(faker.funnyName().name(), faker.funnyName().name(), LocalDate.parse(creaRandomData()));
            ud.save(utente);
        }
    }
    ...
    public static void creazioneMezzoDiTrasporto(MezzoTraspDAO mzd, TrattaDAO td, Random rndm){
        for (int i = 0; i < rndm.nextInt(50, 101); i++) {
            ...
            if (n % 2 == 0){
                MezzoDiTrasporto mezzoDiTrasporto = new MezzoDiTrasporto(TipoDiMezzo.AUTOBUS, rndm.nextLong(40, 101), td.getAllTratte().get(rndm.nextInt(0, td.getAllTratte().size())), n%2==0 ? Stato.IN_SERVIZIO : Stato.IN_MANUTENZIONE);
                mzd.save(mezzoDiTrasporto);
            } else {
                MezzoDiTrasporto mezzoDiTrasporto = new MezzoDiTrasporto(TipoDiMezzo.TRAM, rndm.nextLong(40, 101), td.getAllTratte().get(rndm.nextInt(0, td.getAllTratte().size())), n%2==0 ? Stato.IN_SERVIZIO : Stato.IN_MANUTENZIONE);
                mzd.save(mezzoDiTrasporto);
            }
        }
    }
    ...
    public static void creazionePuntoVedita(PuntoVenditaDAO pd, Faker faker, Random rndm){
        for (int i = 0; i < rndm.nextInt(50, 101); i++) {
            int n = rndm.nextInt(0, 1001);
            if (n % 2 == 0) {
                PuntoVendita puntoVendita = new Distributore(faker.gameOfThrones().character(), faker.country().capital());
                pd.save(puntoVendita);
            } else {
                PuntoVendita puntoVendita = new Rivenditore(faker.gameOfThrones().character(), faker.country().capital());
                pd.save(puntoVendita);
            }
        }
    }
    ...
    public static void creazioneDocumentoVendita(DocumentoVenditaDAO dd, UtenteDAO ud, MezzoTraspDAO mzd, Random rndm){
        ...
        for (int i = 0; i < rndm.nextInt(50, 101); i++) {
            ...
            if (n % 2 == 0) {
                DocumentoVendita documentoVendita = new Biglietto(dataDiRilascio, n%3==0 ? null : dataDiRilascio.plusDays(rndm.nextInt(10, 40)), mzd.getAllMezziDiTrasporti().get(rndm.nextInt(0, mzd.getAllMezziDiTrasporti().size())));
                dd.save(documentoVendita);
            } else {
                DocumentoVendita documentoVendita = new Tessera(dataDiRilascio, ud.getAllUtenti().get(rndm.nextInt(0, ud.getAllUtenti().size())), n%3==0 ? TipoAbbonamento.MENSILE : TipoAbbonamento.SETTIMANALE, dataDiRilascio, n%5==0);
                dd.save(documentoVendita);
            }
        }
    }
    ...
    public static void creazioneManutenzione(ManutenzioneDAO mtd, MezzoTraspDAO mzd, Random rndm){
        for (int i = 0; i < rndm.nextInt(50, 101); i++) {
            ...
            Manutenzione manutenzione = new Manutenzione(dataInizio, dataInizio.plusDays(rndm.nextInt(3, 201)), mzd.getAllMezziDiTrasporti().get(rndm.nextInt(0, mzd.getAllMezziDiTrasporti().size())));
